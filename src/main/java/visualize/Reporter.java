...
public class Reporter {
    ...
    private static void generateBranchCoverageReport(String path, Map<String,String> parameters) throws FileNotFoundException, DocumentException {
        ...
        int branchesSum=Storage.branches.get().values().stream().mapToInt(value -> {
            ...
            return value.stream().mapToInt(BranchStruct::size).reduce(Integer::sum).getAsInt();
        ...
        for(String p:Storage.branches.get().keySet()){
            if(Storage.branches.get().get(p).size()>0){
                table.addCell(p);
                table.addCell(String.format("%d/%d",Storage.exec_branches.get().getOrDefault(p,0),Storage.branches.get().get(p).stream().mapToInt(BranchStruct::size).reduce(Integer::sum).getAsInt()));
                table.addCell(String.valueOf((double) 100*Storage.exec_branches.get().getOrDefault(p,0)/(double)Storage.branches.get().get(p).stream().mapToInt(BranchStruct::size).reduce(Integer::sum).getAsInt()));
            }
        }
